...
import { bookTypeListItem, TypeFingerData, TypeTabsProps, TypeWxDom } from '@src_types';
import React, { CSSProperties, useEffect, useRef, useState } from 'react';
import { View, Text, ITouchEvent, Swiper, SwiperItem, Button, Image } from '@tarojs/components';
import { TaroElement } from '@tarojs/runtime';
import { isType, skip, throttle, trimTitleStyleFn } from '@utils';
import config from '@src_config';

type ExtractName = keyof Pick<TypeFingerData, 'startX' | 'moveX'>;
type TypePartial = Partial<Pick<TypeFingerData, 'isEven' | 'textWidth' | 'titleWidth' | 'windowWidth' | 'underlineStyle' | 'percent' | 'underlineWidth' | 'wxDomArr' | 'titleTop'>>

export default (props: TypeTabsProps) => {
  const global = config.globalData.global
  const { titleStyle, list, nameObj, onModel } = props;
  const [useTitleStyle, setUseTitleStyle] = useState<CSSProperties>(trimTitleStyleFn(titleStyle ?? ""))
  const textRef = useRef<TaroElement & HTMLDivElement>(null)
  const [transformStyle, setTransformStyle] = useState<string | CSSProperties>()
  ...
  const setFingerDataStyleFn = async (node: Record<string, any>): Promise<TypePartial> => {
    let textWidth, titleWidth, isEven, _underlineStyle, windowWidth, percent, underlineWidth, titleTop;
    if (config.globalData.isWeb && textRef.current) {
      ...
      titleTop = (await getDom('.title')).top
  ...
  const getDom = (name: string = '.title-text', isFirst: boolean = true, isCoord: boolean = false) => {
    return new Promise<TypeWxDom>(resolve => {
      const query = Taro.createSelectorQuery()[`select${isFirst ? '' : 'All'}`](name).boundingClientRect()
      isCoord ? query.selectViewport().scrollOffset().exec(res => resolve(res[0])) : query.exec(res => resolve(res[0]))
    })
  }
  ...
  const touchHandler = (status: 0 | 1 | 2, e: ITouchEvent) => {
    ...
    switch (status) {
      ...
      case 1:
        ...
        break;
      case 2:
        !isNextFn() && setTransformStyle(trimTitleStyleFn(transformStyle ?? '', `transform: translateX(${fingerData.skewing}PX)`));
        break;
    }
  };
  ...
  const titleClick = (left: number, i: number) => {
    ...
    if (i !== fingerData.currentI) {
      const currentLeft =
        i === 0 ?
          fingerData.percent : fingerData.isEven ?
            left + (config.globalData.isWeapp ? 11 : config.globalData.isWeb ? 9 : 0) :
            fingerData.percent + (fingerData.textWidth * i)
      fingerData.currentI = i
      fingerData.underlineStyle = trimTitleStyleFn(fingerData.underlineStyle, `left: ${currentLeft}PX`)
      setFingerData({ ...fingerData })
    }
  }
  ...
  const TransitionHandler = async (e: any) => {
    ...
    if (!Array.isArray(fingerData.wxDomArr)) {
      const res: any = await getDom('.title-text', false, true)
    ...
    if (fingerData.currentI !== current) {
      const left = config.globalData.isWeb ?
        ...
        config.globalData.isWeapp ? Math.round(fingerData.wxDomArr![current].left) :
          null;
      if (left !== null) {
        const i = (current - Math.round(global.windowWidth / fingerData.textWidth)) + 1
        setTransformStyle(i >= 0 ? trimTitleStyleFn(transformStyle ?? '', `transform: translateX(-${(fingerData.textWidth * i) + 10}PX)`) : '')
  ...
  const setUseTitleStyleFn = (top: number | any) => {
    top = isType(top, 'event') && config.globalData.isWeb ? top.target.scrollTop : top
    if (!top) return;
    if (!cssNameArr.every(item => Reflect.has(useTitleStyle, item)) && top >= fingerData.titleTop) {
      setUseTitleStyle(trimTitleStyleFn(useTitleStyle, { position: 'sticky', top: 0, zIndex: 100 }))
    } else if (top <= fingerData.titleTop) {
      const currentObj = {}
      Reflect.ownKeys(useTitleStyle).forEach(item => {
        if (!cssNameArr.some(c_item => c_item === item)) Reflect.set(currentObj, item, useTitleStyle[item])
      })
      setUseTitleStyle(currentObj)
    }
  }

  usePageScroll(e => config.globalData.isWeapp && setUseTitleStyleFn(e.scrollTop))

  useEffect(() => {
    if (config.globalData.isWeb) {
      document.querySelector('.taro_page')?.addEventListener('scroll', throttle(setUseTitleStyleFn))
    }
    setTimeoutFn()
  }, [props.list]);
  ...
  return (
    <FlexTitle>
          ...
          {list.map((item, i) => (
            item[nameObj.titleName] ?
              <Text
                ...
                style={{ color: `${i === fingerData.currentI ? '#4FC08D' : ''}` }}
                key={item[nameObj.titleName]}
                onClick={
                  (e: any) => {
                    titleClick(config.globalData.isWeb ?
                      e.path[0].offsetLeft : config.globalData.isWeapp ?
                        e.mpEvent.currentTarget.offsetLeft : -1,
                      i
                    )
                  }
                }
      ...
      <Swiper className='title-swiper' current={fingerData.currentI} onAnimationFinish={TransitionHandler}>
        {list.map((item, i) => {
          if (item[nameObj.contentName]) {
            return (
              <SwiperItem className='title-swiper-item' key={i}>
                ...
                {
                  item[nameObj.contentName].map((_item: bookTypeListItem, _i: number) => {
                    const { book_name, book_author, book_img, book_intro, book_id } = _item
                    return (
                      <View
                        key={book_id}
                        className='title-swiper-item-view'
                      >
                        <Image
